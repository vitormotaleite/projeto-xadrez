...
public class ChessMatch {
	...
	public ChessPiece replacePromotedPiece(String type) {
		...
		if(!type.equals("B") && !type.equals("N") && !type.equals("R") && !type.equals("Q")) {
			throw new InvalidParameterException("invalid type promotion.");
		}
	...
	private void undoMove(Position source, Position target, Piece capturedPiece) {
		Piece p = (ChessPiece)board.removePiece(target);
	...
	private void validateSourcePosition(Position position) {
		...
		if(!board.piece(position).isThereAnyPossibleMove()) {
			throw new ChessException("there are no moves possible for the chosen piece.");
		}
	}
	
	private void validateTargetPosition(Position source, Position target) {
		if(!board.piece(source).possibleMove(target)) {
			throw new ChessException("the chosen piece cannot move to targete position");
		}
	}
